...
- The animation counter can count either way — the status bar's frame pane shows "current / total" by default and "- remaining / total" in countdown mode; click the pane (or **Animation → Count Down (Remaining Frames)**) to switch, and the pane is sized to the widest text the mode can show.
...
**1.1.16-rc.2 — the fifth report’s judgment round:** the frame pane’s click lands on the frame pane again (the date pane’s birth had moved it), the counter is born at its ceiling width (the 9-to-10 hitch), the truncated gif wraps, the delete pair retires the parked preload, the render legs ask the frame-state guard, the dead arrow now needs a page that changes something, the strip answers the keyboard, and the countdown mode gains a menu seat. See [Changes.txt](Changes.txt).
