...
- **The white band after the Options dialog and a theme switch is closed at the root** — the system side repaints the frame and the common control classes in light asynchronously (the app-mode policy flush, the theme broadcast sweep), while the app-level "dark answer" never flipped — and every re-apply was gated on exactly that flip, so the system's light repaint survived: a light caption over a dark body in the field screenshot. The delayed re-check timer and `WM_THEMECHANGED` now re-apply the chrome **unconditionally** (the sweep is idempotent), the Options dark-mode combo schedules the assert after its app-mode flush, and the Options dialog re-runs its own full dark pass on a one-shot timer so a creation moment that read an unsettled state heals itself. The main window also erases with the dark chrome face in the dark UI, so a paint that bypasses the handlers shows dark, never the light class brush.
- **The Options navigation contrast** — the left tree kept the light explorer class's own gray for its labels on the dark face (a themed tree ignores the text-color message); the dark dialog walk now pins the tree face and label colors per pass — the same values the dialog init applies — and hands them back to the system on the light flip.

**1.1.12-rc.3 — the architecture split and the installer metafile associations:**

- **The monolith is gone** — `src/viv.c` was 21,129 lines / 536 functions in one translation unit, with every bug fix of the fork landing somewhere inside it. It is now a 4,700-line core (the window procedure, startup/shutdown, the command line, the shared state definitions) plus **eleven domain modules** — `viv_recent`, `viv_playlist`, `viv_load`, `viv_anim`, `viv_render`, `viv_chrome`, `viv_dark`, `viv_dialogs`, `viv_view`, `viv_install`, `viv_menu` — and a `viv_state.h` shared-context layer, every module under 3,000 lines. The split is a **pure physical move**: function bodies are byte-identical, the only edits are `static` prefixes dropped on the 163 cross-module exports and the 113 shared-state externs; the design and the dialectic live in `docs/architecture/viv-split-spec.md` and `-plan.md`. **Performance and size are unchanged by construction** — the project already compiles with `/GL` whole-program optimization, so cross-module inlining survives the split, and exported symbols never enter the release binary. The three guard suites now read the code as `viv.c` spliced with every domain module, so all 140+ shape guards keep covering the moved code, and new R70 guards pin the residual size, the per-domain caps, the pure-move window (declarations only, no code growth) and the props registration. Five CI catches shaped the landing (three cross-module exports the one-shot move left static, count macros that measured unsized externs — MSVC resolves those to zero, silently stopping five loops in the domain modules — and a fumbled line join that swallowed a declaration into a comment); the count macros are literals now, pinned to their tables by negative-subscript typedef asserts, and every class is guarded.
- **The installer offers the metafile associations** — EMF and WMF joined the viewer's association table in 1.1.12 (open dialog, Everything search, command line, Options checkboxes), but the setup's association page never grew the two checkboxes, so a setup install silently skipped them. The page now carries **EMF and WMF** (second column, both languages); the exe-side install switches were already generic over the extension table, so `/emf`, `/noemf`, `/wmf` and `/nowmf` behave like every other format.
- **Housekeeping** — the vendored libwebp drops its non-Windows build systems, fuzzers and docs (62 files this repository never built); `Changes.txt` keeps its CRLF discipline back (an R69 regression, caught by the simulation suite).

**1.1.12-rc.2 — the About dialog hard code fixes and the white band fix:**

- **The About band chrome lives in the dialog template (1.1.12)** — the About dialog painted its dark-mode band chrome at runtime in `WM_PAINT`: three fillrect passes whose band edges were pixel literals (48 and 46 at the 96-DPI design point, scaled by the system DPI fraction), while the dialog template positions the buttons in dialog units — two coordinate systems that only met at 96 DPI, so at any other scale (and after the font rounds made the dialog font per-DPI) the separator lines drifted off the button strip and the band boundaries crossed the controls. The band geometry now lives in the resource template — two 1-du line controls and a 24-du strip control declared before the buttons, so they sit under them in the z order — the colors travel the control color replies with the same palettes (the dark 0x45/0x70 separator lines and the 0x25 strip face, the light 0xEC lines and the white strip), the dialog face answers through the `WM_CTLCOLORDLG` pipeline, and the `WM_PAINT` case is gone: the band follows the template grid at every DPI, in both themes, and the banner keeps its 1.1.11 face (the black band with the white title in the light UI, the shared dark canvas in the dark).
...
- **The white band after a theme flip is gone (1.1.12-rc.2)** — the flip's immediate repaint sweep never carried `RDW_FRAME`, and the official semantics keep the non-client area out of a frameless sweep: no `WM_NCPAINT` meant the dark gap fill that lives in that message never ran, so the system's light strip survived right of the menu items until a manual resize — the sweep carries the frame now. The dark menu bar items measured with a literal 8px air while the light ones take the system metrics, so every flip relayouted the bar (Chinese labels accumulate the drift) — the pad is now probed from the system's own layout (the rect the system gave a top-level item minus its label extent) and rescaled with the DPI. The toolbar strip: the flip never relayouted the rebar/toolbar after the comctl re-metrics (the DPI and language paths do), the width query returned the content span instead of the window the toolbar needs (`TB_GETMAXSIZE` now, the content scan stays as the pre-5.80 fallback), and the rebar class brush — which the register wrapper hardcoded to white for every class, ignoring the argument — is gone: the strip face is painted by theme alone.
