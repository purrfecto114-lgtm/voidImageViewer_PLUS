...
What's new
--------
**1.1.09 — modern UX round, the field-fix rounds and the simulation checks (the current stable):**
...
- **The dark options text reads (1.1.09)** — the checkbox and radio labels paint light in the dark UI through custom draw: the themed button paints its own label color and the WM_CTLCOLORSTATIC text never reaches it (the black checkbox labels at 0.78:1 contrast), so the custom draw replaces the label only and leaves the glyph to the system theme — the check state machine is never touched (an owner-draw flip would replace BS_AUTOCHECKBOX itself and freeze every checkbox; the first 1.1.09 build did exactly that and was withdrawn). Every dialog template declares the same font now — Segoe UI 9pt, one weight, one charset (the pages and the containers used to measure their dialog units with two different charsets behind the obsolete DS_FIXEDSYS flag) — and every drawn control selects its font before it draws, so the light and the dark UI render the same face. The About dialog's bottom band follows the theme too (no more light strip in the dark UI), and the geometry tests walk the pixel grid the font sets.
