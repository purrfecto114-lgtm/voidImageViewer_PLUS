...
What's new
--------
**1.1.14-rc.6 — the budget and baseline round (the current release candidate):**

- **The working-set budget** — a pixel ceiling alone never bounded memory: the loaders now price the peak a load actually holds (the decode canvas, the display DIB and the renderer staging — 12 bytes per pixel) and refuse when it crosses a byte ceiling (~200 mp effective on x64; the 100 mp behavior unchanged on x86). Animations are stricter on every axis: the x64 canvas ceiling drops to 150 mp, the frame count carries its own 10,000-frame ceiling, and the frame array — which holds every decoded frame at once — is bounded in total bytes (2 GB on x64, 400 MB on x86). The refusal carries its own status-line message in both languages instead of a bare "failed to load".
- **The hard kill is a recorded event** — the exit timeout now reports the stage the loader thread died in (open / decode / frames / webp / qoi / wic) and the file before TerminateThread ever runs, the QOI decoder honors the cooperative cancel mid-decode, and the telemetry exists before the last resort is ever retired.
- **The toolchain baseline is machine-verified** — the v145 project file carries the same /sdl /guard:cf and /GUARD:CF line as the shipping v143 one (the ARM64 buffer-security opt-out retires, the leftover safeseh opt-outs go), and both CI pipelines assert the ASLR / DEP / CFG bits on the compiled exes through a pure-PowerShell PE header check — a flag that silently drops out of a project file fails the build, not a user's machine.
- **The release trust chain** — every release asset carries GitHub artifact attestation (build provenance verifiable with `gh attestation verify`), a CodeQL workflow analyzes the fork's own C sources weekly, and the vendored libwebp tree gains an offline verifier (`tools/update_libwebp.py --check`). The collaboration pack lands: SECURITY.md, CONTRIBUTING.md, CODEOWNERS, Dependabot, issue/PR templates and third-party notices; the pixel-golden structure test is archive-safe (a source zip runs the content checks, not the git-state ones).
