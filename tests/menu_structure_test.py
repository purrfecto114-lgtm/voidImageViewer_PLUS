...
def t_second_review_round40():
    ...
    check("clipboard dib paste applies the pixel budget",
          "if (!_viv_pixel_budget_refused(safe_size_mul((SIZE_T)bih->biWidth,(SIZE_T)height),VIV_MAX_IMAGE_PIXELS))" in viv and
...
def t_pixel_budget():
    ...
    check("webp loader refuses through the budget helper with a diagnostic",
          ...
          # r130: twice now - the early features-header gate (before the
          # anim decoder exists) plus the full anim-info gate below it.
          wp.count("VIV_MAX_ANIMATION_PIXELS") == 2)
...
def t_audit_round18():
    ...
    check("gdi+ path checks the budget before any frame allocation",
          "(os_GdipGetImageHeight(image,&first_frame.high) == 0) && (!_viv_pixel_budget_refused(safe_size_mul((SIZE_T)first_frame.wide,(SIZE_T)first_frame.high),VIV_MAX_IMAGE_PIXELS))" in viv)
    check("webp animation path checks the canvas budget before decoding",
          "(!_viv_pixel_budget_refused(safe_size_mul((SIZE_T)anim_info.canvas_width,(SIZE_T)anim_info.canvas_height),VIV_MAX_ANIMATION_PIXELS))" in webpc)
    check("webp still path checks the budget before the decode allocates",
          "if (!_viv_pixel_budget_refused(safe_size_mul((SIZE_T)features.width,(SIZE_T)features.height),VIV_MAX_IMAGE_PIXELS))" in webpc)
...
def t_split_architecture_round70():
    ...
    check("the spliced total stays in the growth window",
          21130 <= total <= 33900, f"({total})")  # round-102 recalibration (the export module measured 30548); round-108: the cache-set block (measured 31129); round-109: the slot architecture (measured 31138); round-110: the audit response (measured 31170); round-112: the command picker cascade joins the settings domain (measured 31410); round-113: the audit response - the init checks, the ime dissociation, the reply wakeup duty and the offset validator (measured 31613); round-114: the classic options dialogs retire (measured 30523); round-118: the seventh audit response (measured 30741); round-120: the memory and cache round (measured 30961); round-121: the gui limits round (measured 31081); round-122: the resume and chain round (measured 31118); round-123: the report fusion round (measured 31221 after the cross-check patch); round-125: the field response round (measured 31647), then the settings remake (measured 31990); round-126: the settings footer round (measured 32740); round-129: the default-app honesty round (measured 32990); round-132: the toolbar paging round (measured 33405)
...
def t_format_horizons_round94():
    ...
    check("the wic layer serves every frame the codec reports (the r130 multi-frame contract)",
          "user_data,frame_count,wide,high,has_alpha" in wic and
          "for(i=1;i<frame_count;i++)" in wic and
          "VIV_MAX_ANIMATION_PIXELS" in wic and
          "user_data,1,wide,high,has_alpha" not in wic)
...
def t_todo_closure_round96():
    ...
    check("the toolbar walk honors the group mask before the overflow (round-132: the strip pages now)",
          toolbar.index("config_toolbar_groups & (1 <<") <
          toolbar.index("avail = _viv_toolbar_wide - _viv_toolbar_arrow_wide;"))
...
def t_field_sweep_round93():
    ...
    check("the control ring survives (the switch still rings its capsule, round-132: with the capsule's own radius)",
          "_viv_settings_draw_focus_ring(hdc,&ctl->value,(ctl->value.bottom - ctl->value.top) / 2);" in settings)
...
def t_audit_hardening_round103():
    ...
    check("the one merged family prices the working set for every loader (round-132: the copies retired)",
          "(VIV_UINT64)pixels * VIV_IMAGE_WORKING_SET_BYTES_PER_PIXEL > VIV_MAX_IMAGE_BYTES" in vivload and
          "(VIV_UINT64)pixels * VIV_IMAGE_WORKING_SET_BYTES_PER_PIXEL > VIV_MAX_IMAGE_BYTES" not in webp and
          "(VIV_UINT64)pixels * VIV_IMAGE_WORKING_SET_BYTES_PER_PIXEL > VIV_MAX_IMAGE_BYTES" not in wic and
          "(VIV_UINT64)pixels * VIV_IMAGE_WORKING_SET_BYTES_PER_PIXEL > VIV_MAX_IMAGE_BYTES" not in qoi)
    check("the working set refusal is diagnosable from the one family (round-132)",
          "working set budget: refusing a %u mp canvas" in vivload and
          "working set budget: refusing a %u mp canvas" not in webp and
          "working set budget: refusing a %u mp canvas" not in wic and
          "working set budget: refusing a %u mp canvas" not in qoi)
    ...
    check("the webp frame array is gated with the canvas (round-132: through the exported family)",
          "(!_viv_animation_budget_refused(anim_info.frame_count,safe_size_mul((SIZE_T)anim_info.canvas_width,(SIZE_T)anim_info.canvas_height)))" in webp)
    check("the one animation gate prices frames and total bytes (round-132: the webp copy retired)",
          "(VIV_UINT64)frame_count * (VIV_UINT64)canvas_pixels * 16 / 3 > VIV_MAX_ANIMATION_TOTAL_BYTES" in vivload and
          "(VIV_UINT64)frame_count * (VIV_UINT64)canvas_pixels * 16 / 3 > VIV_MAX_ANIMATION_TOTAL_BYTES" not in webp and
          "frame_count > VIV_MAX_ANIMATION_FRAMES" in vivload)
    ...
    check("every budget refusal marks the flag (round-132: the family marks it once, in the load domain)",
          vivload.count("_VIV_LOAD_REFUSED_SET(_viv_load_refused_budget);") == 4 and
          webp.count("_VIV_LOAD_REFUSED_SET(_viv_load_refused_budget);") == 0 and
          wic.count("_VIV_LOAD_REFUSED_SET(_viv_load_refused_budget);") == 0 and
          qoi.count("_VIV_LOAD_REFUSED_SET(_viv_load_refused_budget);") == 0)
...
def t_audit_response_round118():
    ...
    print("the seventh audit response round (1.1.15-rc.8)")
    ...
    check("the bitmap paste prices the copy before it runs",
          "(!_viv_pixel_budget_refused(safe_size_mul((SIZE_T)bm.bmWidth,(SIZE_T)bm.bmHeight),VIV_MAX_IMAGE_PIXELS))" in vivload and
...
def t_memory_and_cache_round120():
    ...
    check("the animation gates price the mipmap's third (round-132: the one family's)",
          "(VIV_UINT64)frame_count * (VIV_UINT64)canvas_pixels * 16 / 3 > VIV_MAX_ANIMATION_TOTAL_BYTES" in vivload and
          "(VIV_UINT64)frame_count * (VIV_UINT64)canvas_pixels * 16 / 3 > VIV_MAX_ANIMATION_TOTAL_BYTES" not in webp)
...
def t_parallel_eval_round128():
    ...
    check("the save-failure boxes reach the release build",
          ...
          c.count("viv_msgbox(_viv_hwnd,caption_wbuf,message_wbuf,MB_OK | MB_ICONWARNING)") == 4)
    ...
    # round 135 (the fourth report's P2-3): the old pin accepted any
    # same-named line after the sweep comment - either retire could die
    # alone and the alpha-zero branch's line kept the pin green. both
    # sites carry their own adjacency now: the comment tail answers for
    # the sweep-done block, the poll-timer pair for the hide block.
    check("the finished sweep retires the wall clock (its own line, in its own block)",
          "a two-second-old tick.\n\t\t\t_zoomui_fade_tick = 0;" in z)
    check("the transparent hide retires the wall clock too (its own line, in its own block)",
    ...
    freeze = {
        "t_slot_architecture_round109": "1.1.15-rc.2",
        "t_audit_response_round110": "1.1.15-rc.2",
        "t_audit_response_round111": "1.1.15-rc.3",
        "t_command_picker_round112": "1.1.15-rc.4",
        "t_audit_response_round113": "1.1.15-rc.6",
        "t_judged_fixes_round114": "1.1.15-rc.6",
        "t_audit_response_round118": "1.1.15-rc.8",
        "t_memory_and_cache_round120": "1.1.15-rc.9",
        "t_gui_limits_round121": "1.1.15-rc.10",
        "t_resume_and_chain_round122": "1.1.15-rc.11",
        "t_report_fusion_round123": "1.1.15-rc.12",
        "t_field_response_round124": "1.1.15-rc.13",
        "t_settings_capacity_round126": "1.1.15-rc.14",
        "t_pill_field_round127": "1.1.15-rc.15",
        # round-132: the table grows with every shipping round now - the
        # r130 sweep dragged the r129 labels (the table had stopped at
        # r127); restored from git and pinned so no sweep can drag them
        # again.
        "t_parallel_eval_round128": "1.1.15-rc.16",
        "t_default_app_honesty_round129": "1.1.15-rc.17",
        # round 135 (the fourth report's P2-7): the table had stopped at
        # r129 AGAIN - the r132/r133 sections were born after it, three
        # sim docstrings and four menu prints sat dragged, and the
        # freeze answered for none of them. every shipping round, the
        # sim docstrings, and the round-labeled prints in both suites
    ...
    ver_pat = r"(1\.1\.1[56](?:-rc\.\d+)?)"
    for fn, want in sorted(freeze.items()):
        m = re.search(r"def " + fn + r"\(\):\n    \"\"\"(.*?)\"\"\"", here, re.S)
        got = re.search(ver_pat, m.group(1)).group(1) if m and re.search(ver_pat, m.group(1)) else None
        check(f"the {fn[len('t_'):]}'s docstring keeps its own version ({want})",
              got == want, str(got))
    # the sim docstrings ride the same table (the r130 sweep dragged
    # three of them while the menu domain sat pinned).
    sim_freeze = {
        "t_sim_memory_cache_round120": "1.1.15-rc.9",
        "t_sim_gui_limits_round121": "1.1.15-rc.10",
        "t_sim_resume_chain_round122": "1.1.15-rc.11",
        "t_sim_report_fusion_round123": "1.1.15-rc.12",
        "t_sim_field_response_round124": "1.1.15-rc.13",
        "t_sim_settings_round126": "1.1.15-rc.14",
    }
    for fn, want in sorted(sim_freeze.items()):
        m = re.search(r"def " + fn + r"\(\):\n    \"\"\"(.*?)\"\"\"", sim_here, re.S)
        got = re.search(ver_pat, m.group(1)).group(1) if m and re.search(ver_pat, m.group(1)) else None
        check(f"the {fn[len('t_'):]}'s docstring keeps its own version ({want})",
              got == want, str(got))
    # and the round-labeled prints (the fourth report's own census:
    # four menu prints and three sim prints sat a round or three away
    # from their docstrings - the docstring domain never saw them).
    check("the round-labeled menu prints keep their own rounds",
          'print("the seventh audit response round (1.1.15-rc.8)")' in here and
          'print("the memory and cache round (1.1.15-rc.9)")' in here and
          'print("the gui limits round (1.1.15-rc.10)")' in here and
          'print("the field response round (1.1.15-rc.13)")' in here)
    check("the round-labeled sim prints keep their own rounds",
          'print("the gui limits round (1.1.15-rc.10)")' in sim_here and
          'print("the field response round (1.1.15-rc.13)")' in sim_here and
          'print("the report fusion round (1.1.15-rc.12)")' in sim_here and
          'print("the memory and cache round (1.1.15-rc.9)")' in sim_here)
...
def t_default_app_honesty_round129():
    ...
    check("the uninstall sweeps both homes (only our value - the shell's stay)",
          ...
          uninstall_body.count("RegDeleteValueW") == 3)
    ...
    #    line the panes show.
    failure_body = body129(ch, "const wchar_t *_viv_status_failure_line(")
...
def t_toolbar_paging_round132():
    ...
    no trace; the designed button gap lands between adjacent faces (the
    dead constant the flush rounded faces exposed); the settings focus
    ring follows the rounded faces it frames (the message box fixed
    this same shape for itself in rc.13); the nav pills inset inside
    their rows; the pill's keyboard focus follows the capsule; and the
    budget refusal family collapses to one exported pair in the load
    domain."""
    tb = read("src/viv_toolbar.c").decode("utf-8", errors="replace")
    gh = read("src/glyphs.h").decode()
    gc = read("src/glyphs.c").decode("utf-8", errors="replace")
    st = read("src/viv_settings.c").decode("utf-8", errors="replace")
    z = read("src/zoomui.c").decode("utf-8", errors="replace")
    load = read("src/viv_load.c").decode("utf-8", errors="replace")
    lh = read("src/viv_load.h").decode()
    webp = read("src/webp.c").decode("utf-8", errors="replace")
    wic = read("src/wic.c").decode("utf-8", errors="replace")
    qoi = read("src/qoi.c").decode("utf-8", errors="replace")

    # 1. THE CHEVRON PAIR (the page arrows' glyphs).
    check("the chevron glyph pair exists for the page arrows",
          "#define GLYPH_CHEVRON_LEFT" in gh and "#define GLYPH_CHEVRON_RIGHT" in gh)
    m = re.search(r"#define GLYPH_COUNT\s+(\d+)", gh)
    check("the glyph count carries the chevron pair (16 to 18)",
          m is not None and m.group(1) == "18")
    i = gc.find("static const _glyphs_glyph_t _glyphs_table[GLYPH_COUNT] =")
    tbl = gc[i:gc.find("};", i)]
    seats = re.findall(r"^\t\{(\d+),(_\w+)\},?\r?$", tbl, re.M)
    check("the glyph table census is the sixteen seats plus the chevron pair",
          len(seats) == 18 and seats[-2] == ("1", "_glyphs_chevron_left_strokes") and
          seats[-1] == ("1", "_glyphs_chevron_right_strokes"), str(seats[-3:]))

    # 2. THE PAGING STATE (the arrows are citizens, not slots).
    check("the strip carries a page state",
          "static int _viv_toolbar_page = 0;" in tb)
    check("the arrows carry their own hover and press state",
          "static int _viv_toolbar_arrow_hover = -1;" in tb and
          "static int _viv_toolbar_arrow_pressed = -1;" in tb)
    check("the fifteen-seat item table is untouched (the arrows are not slots)",
          "#define _VIV_TOOLBAR_ITEM_COUNT 15" in tb and
    ...
    check("the walk places with the same gap rule",
          tb.count("x += _viv_toolbar_button_gap;") == 1 and
          "static void _viv_toolbar_walk_place(int x)" in tb)
    check("the arrows reserve their slots before the right walk",
          "avail = _viv_toolbar_wide - _viv_toolbar_arrow_wide;" in tb)
    check("the design's own gap comment names the flush faces it answers",
          "design's own gap, spent at last" in tb)

    # 4. THE ARROW MACHINERY (the press rule, no timer).
    check("the page step clamps to the group range",
          "page > _VIV_TOOLBAR_GROUP_MAX" in tb and "if (page < 0)" in tb)
    check("the page step re-measures before the full repaint",
          tb.find("_viv_toolbar_measure();", tb.find("static void _viv_toolbar_page_step")) <
          tb.find("InvalidateRect(_viv_toolbar_hwnd,0,FALSE);", tb.find("static void _viv_toolbar_page_step")))
    check("the arrows fire on the release inside (the strip's own rule)",
          "_viv_toolbar_page_step(arrow ? 1 : -1);" in tb and
          "if (_viv_toolbar_arrow_pressed >= 0)" in tb)
    check("the paging carries no timer (a click is one page)",
          "SetTimer" not in tb)
    check("the arrow hit test is defined beside the item hit test",
          tb.find("static int _viv_toolbar_arrow_hit_test(") < tb.find("static int _viv_toolbar_hit_test("))
    check("the capture loss drops the arrow press like the item press",
          tb.count("case WM_CAPTURECHANGED:") == 1 and
          tb.find("_viv_toolbar_arrow_pressed = -1;", tb.find("case WM_CAPTURECHANGED:")) !=
          tb.rfind("_viv_toolbar_arrow_pressed = -1;"))

    # 5. THE SETTINGS FOCUS RING (rounded like the faces it frames).
    check("the settings focus ring carries a radius parameter",
          "static void _viv_settings_draw_focus_ring(HDC hdc,const RECT *rect,int radius)" in st)
    check("a rounded face gets a rounded ring one pixel outside it",
          "RoundRect(hdc,rect->left - 1,rect->top - 1,rect->right + 1,rect->bottom + 1,(radius + 1) * 2,(radius + 1) * 2);" in st)
    check("the square ring survives where the face is square",
          "Rectangle(hdc,rect->left - 1,rect->top - 1,rect->right + 1,rect->bottom + 1);" in st)
    check("the button ring carries the button radius",
          "_viv_settings_draw_focus_ring(hdc,&ctl->rect,_viv_settings_dip(6));" in st)
    check("the dropdown ring carries the dropdown radius",
          "_viv_settings_draw_focus_ring(hdc,&ctl->value,_viv_settings_dip(4));" in st)
    check("the switch ring carries the capsule radius",
          "_viv_settings_draw_focus_ring(hdc,&ctl->value,(ctl->value.bottom - ctl->value.top) / 2);" in st)
    check("the checkbox ring carries the box radius",
          "_viv_settings_draw_focus_ring(hdc,&box,_viv_settings_dip(3));" in st)
    check("the color swatch and the accent row keep the square ring",
          st.count("_viv_settings_draw_focus_ring(hdc,&ctl->value,0);") == 1 and
          st.count("_viv_settings_draw_focus_ring(hdc,&ctl->rect,0);") == 1)
    check("the nav pill insets inside its row (stacked pills never share an edge)",
          "InflateRect(&rect,0,-_viv_settings_dip(2));" in st)
    check("the nav ring rides the inset pill",
          "_viv_settings_draw_focus_ring(hdc,&rect,_viv_settings_dip(6));" in st)

    # 6. THE PILL FOCUS (the capsule, not the dotted square).
    check("the pill's dotted focus square is gone",
          "DrawFocusRect" not in z)
    check("the pill's focus is an accent ring that follows the stadium",
          "static void _zoomui_draw_focus_ring(" in z and
          z.count("viv_theme_color(VIV_TK_ACCENT)") >= 1)

    # 7. THE BUDGET FAMILY (one pair in the load domain).
    check("the pixel budget gate is one exported function in the load domain",
          "int _viv_pixel_budget_refused(SIZE_T pixels,SIZE_T ceiling)" in load and
          "int _viv_pixel_budget_refused(SIZE_T pixels,SIZE_T ceiling);" in lh)
    check("the animation budget gate is one exported function likewise",
          "int _viv_animation_budget_refused(DWORD frame_count,SIZE_T canvas_pixels)" in load and
          "int _viv_animation_budget_refused(DWORD frame_count,SIZE_T canvas_pixels);" in lh)
    check("the decoders carry no copies of their own",
          "static int _pixel_budget_refused" not in webp and
          "static int _animation_budget_refused" not in webp and
          "static int _pixel_budget_refused" not in wic and
          "static int _animation_budget_refused" not in wic and
          "static int _pixel_budget_refused" not in qoi)
    check("every decoder calls the exported pair",
          "_viv_pixel_budget_refused(" in webp and "_viv_animation_budget_refused(" in webp and
          "_viv_pixel_budget_refused(" in wic and "_viv_animation_budget_refused(" in wic and
          "_viv_pixel_budget_refused(" in qoi)
    check("the decoders include the load domain header",
          '#include "viv_load.h"' in webp and '#include "viv_load.h"' in wic and
          '#include "viv_load.h"' in qoi)
    check("the merged gate keeps the working-set second face (condition and diagnostic)",
          load.count("VIV_IMAGE_WORKING_SET_BYTES_PER_PIXEL") == 2 and
...
def t_stable_promotion_round133():
    """The stable promotion round's contract. the 1.1.15 arc's
    nineteen candidates converge on the stable mark: the version
    string drops the rc suffix (VERSION_TYPE stays empty - the string
    is the identity, the tag, the ci validate leg and the guard
    suites all compare against it), the build ticks to 99, the
    changelog's top entry presents itself as a stable, the readme
    twins promote the current-stable block and roll the previous
    stable into its past tense, the security table answers 1.1.15 as
    the supported line with the rc row retired, and the release
    workflow routes the bare tag to a full release (the prerelease
    case matches rc and beta tags only - the promotion is a routing
    fact the workflow already knew)."""
    version = read("src/version.h").decode("utf-8", errors="replace")
    changes = read("Changes.txt").decode("utf-8", errors="replace")
    readme = read("README.md").decode("utf-8", errors="replace")
    readme_cn = read("README_CN.md").decode("utf-8", errors="replace")
    ...
    check("the readme_cn carries one stable block and no candidate block",
    ...
    check("the workflow routes the bare tag to a full release",
          "*-rc.*|*-beta.*) args+=(--prerelease) ;;" in workflow and
          "the banner follows the phase" in workflow)


def t_fusion4_round135():
    """The fourth fusion report's response round (1.1.16-rc.1): the
    zombie class tree, the dead-arrow skip, the failure's face in the
    view, the exit-code zero legs, the backup's merged view, the
    honest read's third answer, the delete's in-flight pair, the ini's
    two last silent legs, the destroy list's scroll ride, the ulw
    pair, the value-face binding table, and the doc pairs the sweeps
    broke. every pin answers to code anchors, not comments."""
    print("the fourth fusion response round (1.1.16-rc.1)")
    viv = read("src/viv.c").decode("latin-1")
    install = read("src/viv_install.c").decode("latin-1")
    os_c = read("src/os.c").decode("latin-1")
    os_h = read("src/os.h").decode("latin-1")
    view = read("src/viv_view.c").decode("latin-1")
    config = read("src/config.c").decode("latin-1")
    settings = read("src/viv_settings.c").decode("latin-1")
    tb = read("src/viv_toolbar.c").decode("latin-1")
    chrome = read("src/viv_chrome.c").decode("latin-1")
    chrome_h = read("src/viv_chrome.h").decode("latin-1")
    wnd = read("src/viv_wndproc.c").decode("latin-1")
    z = read("src/zoomui.c").decode("latin-1")
    ns = read("nsis/installer.nsi").decode("latin-1")
    readme = read("README.md").decode("utf-8", errors="replace")
    readme_cn = read("README_CN.md").decode("utf-8", errors="replace")
    changes = read("Changes.txt").decode("utf-8", errors="replace")

    # 1. THE ZOMBIE CLASS TREE (P1-1): the bare regdeletekey could
    #    never remove the progid it installed (subkeys refuse it), so
    #    every uninstall left the command pointing at the removed exe
    #    while both honest reads answered all-clear.
    check("the uninstall deletes the class tree (the recursive face)",
          "if (!os_delete_key_tree(HKEY_CURRENT_USER,key))" in install and
          "RegDeleteKey(HKEY_CURRENT_USER,key);" not in install)
    check("the tree delete follows the house lazy-resolve pattern",
          'GetProcAddress(module,"RegDeleteTreeW")' in os_c and
          "typedef LONG (__stdcall *OS_RegDeleteTreeW_fn)(HKEY key,const wchar_t *name);" in os_c)
    check("the pre-vista walk enumerates before it deletes",
          "RegEnumKeyExW(hkey,index,subname,&size,0,0,0,0);" in os_c and
          "if (!os_delete_key_tree(hkey,subname))" in os_c)
    check("the os header carries the tree declaration beside its ex sibling",
          "int os_delete_key_tree(HKEY hkey,const wchar_t *name);" in os_h and
          "int os_reg_delete_key_ex(HKEY hkey,const wchar_t *name,REGSAM access);" in os_h)
    check("the true orphan is retired (zero definitions ever answered it)",
          "os_get_monitor_rect_from_window" not in os_h and
          "os_get_monitor_rect_from_window" not in os_c)
    check("the deliberate dark probe keeps its seat (a recorded keep, not residue)",
          "int os_dark_controls_supported(void);" in os_h and
          "int os_dark_controls_supported(void)" in os_c)

    # 2. THE DEAD ARROW (P2-1, the fifth dead machine): the step skips
    #    the groups the mask killed - the navigation group unchecked
    #    under a narrow window made page one a dead step that clamped
    #    home with the chevron still lit.
    check("the live-group predicate reads the mask and the table",
          "static int _viv_toolbar_group_live(int group)" in tb and
          "config_toolbar_groups & (1 << group)" in tb)
    check("the forward step skips the dead groups",
          "while ((page <= _VIV_TOOLBAR_GROUP_MAX) && (!_viv_toolbar_group_live(page)))" in tb)
    check("the back step skips them too (or goes home)",
          "while ((page > 0) && (!_viv_toolbar_group_live(page)))" in tb)
    check("the dead-step bounce is gone (the old clamp-first shape)",
          "page = _viv_toolbar_page + ((dir > 0) ? 1 : -1);" not in tb)
    check("the no-live-group turn owes nothing (the arrow stays honest)",
          "// no live group ahead: the arrow owes no turn." in tb)

    # 3. THE FAILURE'S FACE (P2-2, the sixth dead machine): the three
    #    presets that strip the caption left the title borrow riding a
    #    window text nobody could see - the view itself carries the
    #    line now.
    check("the failure line is the shared face (exported for the paint)",
          "const wchar_t *_viv_status_failure_line(wchar_t *buf);" in chrome_h and
          "const wchar_t *_viv_status_failure_line(wchar_t *buf)" in chrome)
    check("the paint gate lives where the paint state lives (the wndproc call)",
          "if ((!_viv_slot_current.frame_count) && (!_viv_status_hwnd))" in wnd and
          "_viv_paint_failure_face(paint_hdc,view_top,wide,high);" in wnd)
    check("the face body lives in the chrome unit (the face-owning domain)",
          "void _viv_paint_failure_face(HDC hdc,int view_top,int wide,int high)" in chrome and
          "DrawTextW(hdc,failure_buf,-1,&face_rect,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS|DT_NOPREFIX);" in chrome and
          "void _viv_paint_failure_face(HDC hdc,int view_top,int wide,int high);" in chrome_h)
    check("the borrow invalidates the view (the face moves onto the screen)",
          "InvalidateRect(_viv_hwnd,0,FALSE);" in chrome.split("a failed load has no file on screen; the strip says why.")[1][:600])
    check("the view face rides the menu font and the theme's text token",
          "face_font = _viv_menu_font();" in chrome and
          "SetTextColor(hdc,viv_theme_color(VIV_TK_TEXT));" in chrome)

    # 4. THE EXIT CODES (P1-2): the two zero legs carried no pin -
    #    either could flip to one and every suite stayed green while
    ...
    # 5. THE BACKUP'S MERGED VIEW (P2-12): the machine owner lives in
    #    the hklm half - the user-side read alone captured an empty
    #    string that restored as a shadow over the machine default.
    check("the capture reads the merged view before the empty backup",
          "RegOpenKeyExW(HKEY_CLASSES_ROOT,dot_association,0,KEY_QUERY_VALUE,&hkey_cr)" in install)
    check("the restore refuses to write the empty shadow",
          "RegDeleteValueW(hkey,0);" in install)

    # 6. THE HONEST READ'S THIRD ANSWER (P3-6): a refused userchoice
    #    read is not a missing lock - the mdm profiles deny the read
    #    while the shell honors it.
    check("the refused userchoice read lands with the lock",
          "open_ret = RegOpenKeyExW(HKEY_CURRENT_USER,key,0,KEY_QUERY_VALUE,&hkey);" in install and
          "if (open_ret == ERROR_ACCESS_DENIED)" in install)

    # 7. THE DELETE'S IN-FLIGHT PAIR (P2-13): the paste and blank
    #    paths set the shape; the delete only freed the queue.
    check("the delete retires the in-flight load before the reload",
          "_viv_load_image_allow_draw = 0;\r\n\t\t\t\tInterlockedExchange(&_viv_load_image_terminate,1);\r\n\r\n\t\t\t\t// rc.16: a next file queued" in view)

    # 8. THE INI'S LAST TWO SILENT LEGS (P3-1): the refused temp file
    #    and the failed appdata root answered nothing at all.
    check("the refused temp file gets the box the other legs carry",
          'debug_printf("config save: the temp file refused to open (error %u) - no write was attempted\\r\\n",GetLastError());' in config)
    check("the appdata path failure gets the same box",
          'debug_printf("config save: the appdata path failed (error %u)\\r\\n",GetLastError());' in config)

    # 9. THE DESTROY LIST'S SCROLL RIDE (P3-2): the capture-lost clear
    #    covers the held-capture case; the list covers the rest.
    check("the destroy list carries the scroll ride",
          "_viv_settings_capture_active = 0;\r\n\t\t\t_viv_settings_scroll_drag = 0;\r\n\t\t\t_viv_settings_scroll_grab = 0;" in settings)

    # 10. THE ULW PAIR (P2-4): the probe's unpoison toggle (the msdn
    #     contract) and the submit's refusal self-heal carried no pin.
    check("the probe unpoison toggle is pinned (the exclusivity contract)",
          "SetWindowLong(_zoomui_hwnd,GWL_EXSTYLE,exstyle & ~WS_EX_LAYERED);\r\n\t\t\t\t\tSetWindowLong(_zoomui_hwnd,GWL_EXSTYLE,exstyle | WS_EX_LAYERED);" in z)
    check("the submit refusal self-heals (the layered leg retires)",
          "_zoomui_layered_ok = 0;\r\n\t\t\t\r\n\t\t\tSetWindowLong(_zoomui_hwnd,GWL_EXSTYLE,GetWindowLong(_zoomui_hwnd,GWL_EXSTYLE) & ~WS_EX_LAYERED);" in z)

    # 11. THE VALUE-FACE TABLE (P2-5): the label face and the run face
    #     carried binding tables; the value face answered to a
    #     case-presence walk, so a value case reading the wrong config
    #     stayed green.
    value_bind = {
        "LANGUAGE": "config_language",
        "THEME": "config_dark_mode",
        ...
        "WHEEL": "config_mouse_wheel_action",
        "CTRLWHEEL": "config_ctrl_mouse_wheel_action",
        "COMMAND": "_viv_settings_command_index",
        "KEYS": "_viv_settings_key_index",
    }
    s = settings
    vi = s.find("static void _viv_settings_draw_dropdown(HDC hdc,const _viv_settings_ctl_t *ctl,int hot,int focus)\r\n{")
    vdraw = s[vi:]
    vdraw = vdraw.replace("\r\n", "\n")
    vdraw = vdraw[:vdraw.find("\n}\n")]
    seen = 0
    for mv in re.finditer(r"case _VIV_SETTINGS_ID_(\w+):\n(.*?)break;", vdraw, re.S):
        name = mv.group(1)
        if name in value_bind:
            seen += 1
            check(f"the dropdown {name.lower()} value binds its own source",
                  value_bind[name] in mv.group(2), name)
    check("the value table covers the twelve config-read faces (the counts ride their own ladder)",
          seen == 12, str(seen))

    # 12. THE DOC PAIRS (P2-9/10/11 + the example drift): the twins,
    #     the rc.11 header, the rc.15 row, the rc.9 bullets, and the
    #     example filenames that stopped moving with the stable.
    check("the options twin names the settings page in both tongues",
          "in Settings keeps its meaning" in readme and
          "\u8bbe\u7f6e\u4e2d\u7684" in readme_cn)
    ...
    # 13. THE REBOOT NOTE (P3-8): the /REBOOTOK pair owed the user a
    #     sentence.
    check("the reboot flag owes the user a sentence (both tongues)",
          "IfRebootFlag 0 no_reboot_note" in ns and
          'LangString MsgRebootRequired ${LANG_ENGLISH}' in ns and
