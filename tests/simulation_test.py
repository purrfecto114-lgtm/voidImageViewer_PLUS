...
def t_sim_version_117():
    ...
    check("the version quad is 1.1.13.67",
          (major, minor, rev, build) == (1, 1, 13, 67), str((major, minor, rev, build)))
    check("the release identity string is 1.1.13-rc.7",
          vstr is not None and vstr.group(1) == "1.1.13-rc.7", vstr.group(1) if vstr else None)
    ...
    check("the changelog top entry is the 1.1.13-rc.7 field sweep round",
          top == "Pre-release: Version 1.1.13-rc.7 (the field sweep round)", top)
    ...
    check("the readme candidate line says 1.1.13-rc.7",
          "**1.1.13-rc.7 —" in readme and "(the current release candidate):**" in readme)
