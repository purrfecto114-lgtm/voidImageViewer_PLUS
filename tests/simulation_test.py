...
def t_sim_version_117():
    ...
    check("the version quad is 1.1.13.68",
          (major, minor, rev, build) == (1, 1, 13, 68), str((major, minor, rev, build)))
    check("the release identity string is 1.1.13",
          vstr is not None and vstr.group(1) == "1.1.13", vstr.group(1) if vstr else None)
    ...
    check("the changelog top entry is the 1.1.13 format horizons stable",
          top == "Stable: Version 1.1.13 (the format horizons round)", top)
    ...
    check("the readme rides without a candidate slot (the stable took it)",
          "**1.1.13-rc.7** —" in readme and
          "**1.1.13-rc.7 —" not in readme and
          "(the current release candidate):**" not in readme)
