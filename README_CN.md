...
最新动态
--------
**1.1.15-rc.14 — 设置页脚轮（当前候选版）：**

- 4K-300% 报告全数落地——常规页的页脚按钮曾被静默丢弃（三十三个活控件挤三十座数组；容量升至四十席，普查守卫逐页对账），窗口高度钳制改问自己所在显示器的工作区。
- 工作区不够高时（720p@100%、1080p@150%、4K@300%）设置内容在固定页脚下滚动——滚轮、翻页键、可拖动滑块，键盘走焦自动滚入视野。
- 页脚新增**应用**——不关窗提交，无改动时置灰（脏态灯）；之后的 Esc 或取消以已应用状态为回滚基线。悬浮控制条现在是真热切换——rc.13 的接线本来就通，缺的按钮才是断点。
- 键盘修复：方向键环绕到导航列不再瞬移切页，回车优先响应聚焦按钮，导航列绘制焦点环，文件菜单项终于和窗口同名（设置）。
