...
最新动态
--------
**1.1.15-rc.16 — 并行评估轮（当前候选版）：**

- 四路只读代理清扫 rc.15 树（递延台账、六维审查、变异普查、卫生走查），主线程在动一个字节前亲读每条承重断言。
- **行为修复**（五条幸存者）：设置滚动条不再被 Alt-Tab 抢走捕获后幽灵拖动；删除图片时撤销排队中的下一张解码（一次步进不再解码两个文件）；设置保存失败弹窗告知（发行版不再是静默）；旋转改写磁盘文件前先确认（壁纸一直有确认）；`/everything` 与 `/random` 回到 usage 帮助。
- **守卫补牙**（变异普查七名幸存者）：渐隐停摆阈值、药丸命令表与字形表（对调 prev/next 后全套件曾全绿）、渐隐退役复位、下拉 label/run 绑定——设置 census 长出第四、第五张面，渐隐/滚动模型常数改为从源码提取。
- **卫生批**：历史轮 docstring 冻结（防版本横扫再拖动）、五个死枚举与两个死 os 函数退役、经验台账 rc.12 节归位时序。

**1.1.15-rc.15 — 药丸实地轮：** 悬浮条 90% 缩放、墙钟渐隐（机制层修复闪烁）、窗口幽灵修复。见 [Changes.txt](Changes.txt)。
