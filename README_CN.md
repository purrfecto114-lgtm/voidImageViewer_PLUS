...
最新动态
--------
**1.1.15-rc.19 — 工具栏分页轮（当前候选版）：**

- 窄窗口不再截断工具栏：内容超出窗口时右端出现翻页箭头，一次点击翻一组（分隔符自己的单位——按钮永不被切成半截）；翻页后左端出现回翻箭头，打开按钮永不翻走，窗口拉宽越过溢出即复位。
- 设计图里 8dip 的按钮间距终于落地——常量被缩放被读取却从未被使用，紧贴的圆角悬停面读起来像一个控件渗进下一个。
- 设置页焦点环跟随它框住的圆角面（方形环的四角戳出每一个圆角——消息框在 rc.13 已为自己修过同款形状），导航药丸在行内内缩，浮动条键盘焦点改为跟随胶囊的强调色环。
- 预算拒绝家族归一：像素闸与动画闸只活在加载域一处，webp、wic、qoi 三个解码器都调用导出的一对函数。
