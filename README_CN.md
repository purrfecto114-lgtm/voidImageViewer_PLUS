...
最新动态
--------
**1.1.15-rc.18 — 合并报告裁决轮（当前候选版）：**

- WIC 路径现在递送编解码器报告的每一帧：多帧 HEIF/AVIF 序列真正播放动画，不再永远停在第一帧；动画预算双闸（webp 同款）把关，统一 100ms 标准延迟承运。
- 帧状态不变量收进单一守卫：所有帧数组读取点先问守卫——空动画拒绝、越界位置修复并记录，-1 旧患（上一张回绕、跳帧回绕、跳尾、定时器读取）闭口。
- 安装器问机器不问自己（GetNativeSystemInfo 取代进程级 wow64 探测）；标准用户默认目录跟随账户：Program Files 写探针失败时默认目录移到用户级 Programs 目录，不再安装中途才失败。
- 加固集：QPC 时钟对同域退化、webp 画布预算先于动画解码器存在、空 bits 的 DIB 与被拒的 Select 跳过该帧、QOI 尾部宽容成为成文契约。
