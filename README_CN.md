...
最新动态
--------
**1.1.15-rc.17 — 默认应用诚实轮（当前候选版）：**

- 关联勾选不再自说自话：Windows 10/11 的默认应用藏在第三方无法签名的 UserChoice 哈希之后。勾选扩展现在会同时在两处注册表登记“打开方式”条目；当系统默认应用仍指向别的程序时，如实说明并提供“设置 > 默认应用”页面直达。
- 注册落地即刻通知外壳（SHChangeNotify——官方文档指出否则资源管理器可能直到重启才感知），卸载对称清扫两处新条目。
- 每种预设下加载失败都有面目：极简/紧凑模式隐藏状态栏，失败文案（未找到、超上限、超预算、普通失败）改走标题栏，直到下一次成功加载。
