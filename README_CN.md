...
最新动态
--------
**1.1.15-rc.12 — 审查融合轮（当前候选版）：**

- 第三轮融合报告的账本整体落地 — 两条 P1 守卫牙（rc.7 的 restore 从「注释可满足的钉」改为代码形状钉、选择器叶 id 算术按计数钉）与十六条 P2 一行修复：链走的空座位拒绝、退出的按已加载计数释放、iconic 几何、隐藏窗展开、消息框钳位、定时器责任回交、粘贴的排队下一张拒绝、菜单/标题栏/光标/另存为四面答显示槽、删除后的重锚扫描、安装器运行期引号拒绝。
- 守卫咬机器不咬提及 — census 扩展到 case 绑定（接错键的 case 从此过不了）、计数梯级边界从源码提取并重放、golden 棘轮与 fail-closed 出口有了自己的钉、模拟词表覆盖完整 winuser 显示词集。
- CI 与安装器信任 — release/codeql 每个作业带超时、tests 工作流不再授予无人消费的写令牌、sha256.txt 以 LF 行尾发布、zip 渠道随附第三方声明、卸载器二段自删 exe 不再留在 %TEMP%。
