...
最新动态
--------
**1.1.15-rc.10 — GUI 限制轮（当前候选版）：**

- 右下角一对信息有了呼吸空间 — 文件时间戳带上真实留白，时间与大小是两条事实，不再黏成一行。
- rc.9 的张数下拉活了 — **缓存张数**与**预加载张数**现在能弹出列表、画出标签与当前值并真正生效（改缓存张数按设计清空环形队列）。
- 窗口变窄时先让位光标读数、后让位文件时间 — 时间与大小这对坚持到最后一刻。
- 缩放编辑框只收四位数字；消息框不再截断命令行用法页（`/language` 与 `/uninstall` 两行回来了）。
