...
最新动态
--------
**1.1.15-rc.11 — 续播与链轮（当前候选版）：**

- **从上次位置继续** 开关画出药丸 — rc.9 加的开关行存在且能切换，但开关本体从未渲染；现在开/关一目了然。
- 预载链不再自我吞噬 — 环容量小于链长时会驱逐链自己的队头（默认单座位缓存上「预载 3 张」每张下一张都从磁盘重读）；现在链走到环容量即停、预载设置自动把缓存升到承诺所需的座位数、走到预载槽已持有的文件直接复用不再重复解码。
- 守卫套件新增控件机器全矩阵巡检 — 每个开关对激活/标签/状态绘制三机器、每个下拉对弹出/标签/值三机器 — 控件存在而机器缺失从此过不了守卫。
