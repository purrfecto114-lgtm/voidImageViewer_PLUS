...
最新动态
--------
**1.1.15-rc.15 — 药丸实地轮（当前候选版）：**

- 悬浮控制条整体缩至 **90%**——48×44 dip 胶囊变为 43×39，托盘边距、格距、分隔线与百分比内边距同乘一对缩放系数。
- 全屏**闪烁**在机制层修复，而非猜测——报告怀疑与录屏软件抢焦点，但代码里根本没有前台抢占；真凶是 15ms 渐隐定时器与 15.6ms 系统时钟分辨率打架，固定步长以每秒 66 次分层提交聚簇成可见跳变。渐隐改为墙钟驱动（步长跟随真实节拍），间隔落在稳定的 30ms 倍频上，提交率减半。
- **窗口模式幽灵**修复——渐隐机器只在全屏自动隐藏模式运转，窗口模式的显示从 alpha 0 起步却无人推进，永远全透明。窗口行现在直接不透明起步，切出全屏/重新开关的每条路径都会把残留 alpha 拉回不透明。
...
手势需要 Windows 7+ 与触控硬件。单指输入保持与鼠标兼容，配置的点击动作不受影响。通过 **查看 → 显示悬浮控制条** 切换浮动控件。捏合可以继续缩到窗口适应之下，最小约为适应/16（对应 16× 缩放上限）——选项中的 `允许缩小` 含义不变。

画布、背景与深色模式
--------

- **窗口 / 全屏背景色** —— 设置 → 查看，或查看菜单取色器；图像周围的衬边与空窗口画布。
