...
- 动画计数器可双向计数 — 状态栏帧窗格默认显示“当前 / 总数”，倒数模式显示“- 剩余 / 总数”；点击窗格（或**动画 → 倒数剩余帧**）切换，窗格按该模式最宽文本预留宽度。
- 窄窗口工具栏分页翻组（按钮永不被切成半截），设计图里的 8dip 按钮间距终于落在相邻面之间。

**1.1.16-rc.2 — 第五轮报告裁决轮：** 帧窗格的点击重新落回帧窗格（日期窗格诞生时曾被打偏）、计数窗格按最大位宽出生（9→10 的卡顿）、截断 GIF 回绕、删除对退役停靠预载、渲染双腿过帧状态守卫、死箭头需要真能改变内容的翻页、工具栏条带响应键盘、倒数模式获得菜单席位。详见 [Changes.txt](Changes.txt)。
**1.1.16-rc.1 — 第四轮融合响应轮：** 僵尫类树倒下（卸载器终于能删掉它装下的 ProgID）、死箭头转动（被屏蔽的组永不成为一页）、失败文案在无标题栏预设下落进视图，以及退出码对、备份合并视图、诚实读第三答与守卫形状齐应第四份报告。见 [Changes.txt](Changes.txt)。
